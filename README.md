...
Leash 把**安全**和**可控**作为核心设计目标——就像给 AI Agent 拴上一根"牵绳"（leash），限制其能力范围，防止越权行为。程序默认没有任何权限——读文件、访问环境、调用 AI 模型等所有副作用都必须通过宿主显式注入的"能力（capability）"完成，并且每一步都被审计。这让 AI Agent 可以在严格受限的沙箱中自主运行，同时仍拥有完成任务所需的工具。
